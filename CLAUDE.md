...
コアコーデック（libopenjp2）はlibm・pthreads以外の外部依存がない。PNG/TIFF/lcms2はCLIツール（opj_compress, opj_decompress, opj_dump）専用。Rust移植では標準ライブラリがlibm・スレッドをカバーするため、コアコーデックの外部依存は `thiserror`（エラー型導出）のみ。
