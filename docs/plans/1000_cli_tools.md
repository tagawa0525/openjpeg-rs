...
| ------------------ | ----------------------- | -------------------------- | ------ |
| `opj_decompress.c` | `bin/opj_decompress.rs` | JPEG 2000復号 → 画像出力   | ~4,600 |
| `opj_dump.c`       | `bin/opj_dump.rs`       | コードストリーム情報表示   | ~1,600 |
| `opj_compress.c`   | `bin/opj_compress.rs`   | 画像入力 → JPEG 2000符号化 | ~4,900 |
| `convert.c/h`      | `bin/convert/`          | 画像フォーマット変換       | ~5,200 |
...
`main.rs`を`opj_decompress`に置き換え。追加バイナリは`[[bin]]`で定義。

```toml
[[bin]]
name = "opj_decompress"
path = "src/bin/opj_decompress.rs"
