...
Phase 800（マルチスレッド）完了後の次フェーズ。DWT・MCT・T1のホットループに`std::arch`（SSE2/AVX2）によるSIMD最適化を追加する。

C版はSSE2/AVX2/AVX-512のコンパイル時分岐で実装。Rust版ではランタイムCPU検出 + `#[target_feature]`によるディスパッチで、スカラフォールバックを常に維持する。

Phase 900が完了すると、DWT垂直パス・MCT変換・DWT水平リフティングがSIMD加速される（マスタープランM6の一部）。
...
| `mct.c: opj_mct_encode/decode()`      | RCT (i32)        | SSE2       | 4サンプル/iter              |
| `mct.c: opj_mct_encode/decode_real()` | ICT (f32)        | SSE        | 4サンプル/iter              |
